...
def inscrever_evento(request, id):
    ...
    if request.method == 'GET':
        ...
        # TODO:Validar se um suario já é um participante
...
def gerar_certificado(request, id):
    ...
    path_fonte = os.path.join(
        settings.BASE_DIR, 'templates/static/evento/fontes/arimo.ttf')
    ...
    for participante in evento.participantes.all():
        ...
        draw.text(
            (816, 849), f'{evento.carga_horaria}:00 horas', font=font_info, fill=(0, 0, 0))
        output = BytesIO()
...
def procurar_certificado(request, id):
    ...
    if not certificado:
        messages.add_message(request, constants.ERROR,
                             'Esse certificado ainda não foi gerado.')
        return redirect(reverse('certificados_evento', kwargs={'id': evento.id}))
